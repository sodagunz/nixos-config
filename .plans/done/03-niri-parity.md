...
1. Inventory the old Hyprland bindings, autostart commands, output settings,
   ...
   and session actions.
1. Compare that inventory with the current `config.kdl` and mark each item as:
   preserved, intentionally changed for Niri, missing, or broken.
1. Fix the daily-use bindings first: terminal, launcher, browser, close window,
   focus/move, workspaces, monitor movement, screenshots, volume, brightness,
   lock, logout, suspend, and power actions.
1. Validate Niri syntax after each group of edits and inspect the Niri user
   service log for rejected bindings or startup commands.
1. Split generated values from the main KDL only when it improves ownership;
   avoid redesigning the entire module during parity work.
1. Record which shell components will later be replaced by Noctalia so they are

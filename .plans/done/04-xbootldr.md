...
1. Create Windows recovery media and confirm the BitLocker recovery key if
   applicable.
1. Disable Windows Fast Startup/hibernation and check the Windows filesystem.
1. Keep a NixOS live USB and a known-good generation available.
1. Confirm exact device names, partition numbers, free space, UUIDs, and mount
...
1. Create a FAT32 partition in the freed space and set its XBOOTLDR type GUID.
1. Format it, copy/preserve any required NixOS boot content, and update the
   hardware/filesystem declarations using UUIDs.
1. Configure systemd-boot with the ESP at `/efi` and XBOOTLDR at `/boot`.
1. Mount both filesystems and run a boot activation.
1. Confirm entries with `bootctl`, reboot once into the new default, and retain
...
- The temporary aggressive initrd compression and very low generation limit can
  be reconsidered after the larger partition is working.

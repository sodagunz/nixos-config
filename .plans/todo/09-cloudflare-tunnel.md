...
Run the existing managed Cloudflare tunnel automatically at boot. Do not restore
Caddy. Store its rotated, per-tunnel token encrypted in Git and deploy only that
token at runtime.

## Plan

1. Confirm the existing managed tunnel ID and rotate its per-tunnel token.
1. Add Agenix as a flake input following stable `nixpkgs`, and include its NixOS
   module only in Filmotheque.
1. Add `secrets/secrets.nix` with Filmotheque's SSH host Ed25519 public key and
   the user's `id_ed25519_local` public key as recipients.
1. Have the user encrypt the existing credentials interactively without the
...
1. Add a `system/services/cloudflared` feature, import it through the server
   ...
   `nixpkgs-unstable`.
1. Enable the feature in Filmotheque with managed tunnel ID
...
1. Add a Git ignore safeguard for plaintext files under `secrets/`, while
   continuing to track `.age` files.
1. Stop the manually launched Cloudflared connector, activate the NixOS
   configuration, and verify the managed connector locally and through the
   public hostname.
1. Reboot Filmotheque and repeat the checks to prove unattended startup. After
...
- The runtime tunnel token exists only under `/run/agenix`, is mode `0400`, and
  systemd copies it into the dynamic service's private credential directory.
...
- The Cloudflare dashboard owns the managed tunnel's public-hostname and origin
  routing; this change does not manage Cloudflare DNS or ingress.
...
- `cert.pem` is needed for administrative tunnel management, not for running the
  existing named tunnel, so it stays outside this repository.
- Follow-up issue `10-caddy-https-origin` replaces the loopback HTTP origin with
  Caddy and strictly verified HTTPS. It also documents the remaining Cloudflare
